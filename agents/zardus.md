...
### Memory Tools Reference (MCP)
| Tool | Use When |
|---|---|
| `mcp_memory_create_entities` | New project, person, tool, concept |
| `mcp_memory_create_relations` | Link entities together |
| `mcp_memory_add_observations` | Any fact, decision, or state change |
| `mcp_memory_search_nodes` | Context recovery, finding related entities |
| `mcp_memory_read_graph` | Full cold-start reconstruction |
| `mcp_memory_open_nodes` | Deep-dive a specific entity |
| `mcp_memory_delete_entities` | Remove obsolete entries |
